...
class Heap {
  ...
  delete() {
    ...
    // call `_shiftDown` to move the new value at the root
    // of the heap to a valid spot, making way for the
    // propper root of the min heap
    this._siftDown(0);
    return min;
  }
  ...
  // be swapped with it's parent's value until it reaches a valid spot
  _bubbleUp(index) {
  ...
  //helper method that checks to see if the value at the given index
  // is in a valid spot in the heap. If it isn't the value will
  // be swapped with it;s smallest child's value until it reaches a valid spot
  _siftDown(index) {
